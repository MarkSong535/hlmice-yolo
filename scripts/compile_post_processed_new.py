...
def distance(p1, p2):
    ...
    _p2 = np.array(p2, dtype=np.float32)*np.array([640,480])
...
for video in videos:
    ...
    data = np.full(
            (DLC_file.shape[0] ,(len(row_structure)) * 5), -np.inf
        ) 
    while sorted_files != []:
        ...
        for object_data in entries:
            ...
            if not id in closest_organize:
                ...
                if not placeholder[id * 5] == -np.inf:
                    if midx-1 >= DLC_file.shape[0]:
                        continue
                    dist_new = distance(DLC_file[midx-1], points[1:3])
                    dist_old = distance(DLC_file[midx-1], placeholder[id * 5 : id * 5 + 2])
